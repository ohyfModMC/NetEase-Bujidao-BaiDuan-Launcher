...
public static class AccountMessage {
    ...
    {
        lock (GameSaveAccountLock) {
            // 获取账号列表 (不触发默认自动登录, 避免副作用)
            var (accountList, accountPath) = GetAccountList1(defaultLogin: false, safeUserId: false);

            // 物理移除目标账号并压缩数组 (原实现置 null 会导致重新加载时遍历崩溃)
            var kept = accountList
                .Where(a => a != null && a.Id != id)
                .ToArray();

            // 写入文件
            File.WriteAllText(accountPath, JsonSerializer.Serialize(kept), Encoding.UTF8);
        }
    }
