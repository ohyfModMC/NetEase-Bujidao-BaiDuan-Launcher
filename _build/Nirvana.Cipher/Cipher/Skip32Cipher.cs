...
public static class Skip32Cipher {
    ...
    private static int G(byte[] key, int k, int w)
    {
        ...
        var g3 = FTable[g2 ^ (key[4 * k % 10] & 0xFF)] ^ g1;
        var g4 = FTable[g3 ^ (key[(4 * k + 1) % 10] & 0xFF)] ^ g2;
        var g5 = FTable[g4 ^ (key[(4 * k + 2) % 10] & 0xFF)] ^ g3;
        var g6 = FTable[g5 ^ (key[(4 * k + 3) % 10] & 0xFF)] ^ g4;
